...
<html>
	...
	<h3>게시판 수정</h3>
	  <form action="./content_view2.jsp">
	    ...
	    <label>공지글등록</label>
	    <input type="radio" id="n_yes" name="notice" value="n_yes"
	    <%=request.getParameter("notice").equals("n_yes")?"checked":"" %>
	    >공지글등록
	    <input type="radio" id="n_no" name="notice" value="n_no"
	    <%=request.getParameter("notice").equals("n_no")?"checked":"" %>
	    >공지글취소<br>
	    <label>글카테고리</label>
	    <input type="checkbox" id="screen" name="category" value="screen"
	    <%=request.getParameter("category").contains("screen")?"checked":"" %>
	    >영화<br>
